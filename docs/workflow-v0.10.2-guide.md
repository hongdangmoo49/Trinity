...
| Execution Matrix | work package table, task 확대 토글, execution log 표시 |
| Settings | theme, color compatibility, density, 시작 로고 motion/glyph, agent/central model 기본값 저장 |
...
Execution Matrix에서는 `f` 키 또는 `Expand Tasks` 버튼으로 task 목록 영역을 확대한다.
확대 상태에서는 package list가 화면 대부분을 사용하고, task 컬럼이 더 긴 문자열을
표시하며, execution log는 하단의 작은 영역으로 유지된다. 같은 조작을 다시 수행하면
기본 compact layout으로 돌아간다.
