...
- Add a compact preview for saved `mode == "new"` project context.
...
- Keep existing-project UI unchanged by rendering no preview for existing context.
...
- Do not change the persisted `.trinity/project-intake.*` schema.

## Design

Add a shared `project_plan_preview_label` helper next to the existing workspace
label helpers. It loads the saved project context and returns an empty string
unless the saved context is for the selected new-project target.
...
status formats the same saved context state. No Start/Nexus widget is required.

## Tests

- The shared preview helper returns an English and Korean preview for new context.
- Existing-project context returns no preview.
