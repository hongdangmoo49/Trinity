"""Legacy tmux pane I/O for interactive/debug transport."""
...
class TmuxPane:
    ...
    def send_text_heredoc(self, text: str, marker: str = "TRINITY_EOF") -> None:
        """Inject multi-line text safely via a tmux buffer paste."""
        import os
        import tempfile
    ...
    def send_keys(self, keys: str) -> None:
        """Send raw keys without automatic Enter."""
        subprocess.run(
    ...
    def capture_text(self, lines: int = -100) -> str:
        """Capture pane output as a single string."""
        result = subprocess.run(
            ["tmux", "capture-pane", "-t", self.pane_id, "-p", "-S", str(lines)],
            capture_output=True,
            text=True,
