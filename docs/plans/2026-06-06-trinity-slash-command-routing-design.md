...
| `/project` | `[workspace\|analyze\|validation]` | 로컬/UI 조회 | 없음 | 없음 | 인자가 없으면 Start/Nexus의 긴 프로젝트 진단을 local command 결과로 표시한다. 하위 명령은 에이전트를 호출하지 않고 workspace picker, 프로젝트 분석/검증 modal을 연다. |
