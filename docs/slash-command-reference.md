...
1. 조회 명령의 중앙 Markdown 출력을 Textual table/panel/inspector 위젯으로 확장한다.
2. `/resume` archive 목록을 Textual-native selector/modal로 표시한다.
3. command registry와 문서 표가 어긋나지 않도록 자동 검증을 추가한다.
4. 세션 메모리 변경 명령과 config persistence 명령을 분리해 사용자에게 저장 여부를
   명확히 보여준다.
