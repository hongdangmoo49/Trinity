...
| `/settings` | 없음 | 로컬/UI 조회 | 없음 | 없음 | Textual Workbench 설정 화면으로 전환한다. plain TUI에서는 기본 `trinity` Workbench에서 `/settings` 또는 `Ctrl+,`를 사용하라는 안내를 표시한다. |
...
| `/model` | 없음 | 로컬/UI 조회 | 없음 | 현재 Textual selector 모델 override 변경 | Textual Start/Nexus에서 에이전트별 다음 요청 모델 modal을 연다. 선택한 모델은 다음 prompt/follow-up의 model override로만 사용되며, provider를 호출하지 않고 설정 파일에도 저장하지 않는다. |
