...
현재 `/report`는 plain TUI 실행 핸들러에 있지만 `TRINITY_COMMANDS` 자동완성 목록과
Textual command description에는 빠져 있다. 발견성 개선 대상이다.
...
1. Textual Workbench의 `/` 팔레트는 명령 실행기가 아니다. `/status`를 제출하면 status
   화면이 아니라 workflow follow-up으로 들어갈 수 있다.
2. `/report`는 plain TUI 핸들러가 있지만 자동완성/Textual 팔레트 목록에는 없다.
3. `/exit`, `/q`는 plain TUI alias로 동작하지만 자동완성 목록에는 없다.
4. `Ctrl+P`는 command palette binding 문구가 있으나 실제 팔레트는 `/` 입력으로만 열린다.
5. `/rounds`, `/agent`, `/caveman`은 현재 세션 메모리만 바꾸며 config 파일에 저장하지 않는다.
6. README의 기존 TUI 명령 표는 전체 명령 목록을 다 담지 못해 이 문서를 상세 기준으로 둔다.
...
4. `/report`, `/exit`, `/q` 같은 alias와 discoverability gap을 registry 테스트로 막는다.
5. 세션 메모리 변경 명령과 config persistence 명령을 분리해 사용자에게 저장 여부를
   명확히 보여준다.
