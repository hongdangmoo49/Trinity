...
| :--- | :--- | :--- | :--- |
| 로컬/UI 조회 | 없음 | `/help`, `/status`, `/context`, `/project`, `/providers`, `/workspace`, `/settings`, `/model`, `/history`, `/workflow`, `/questions`, `/decisions`, `/packages`, `/subtasks`, `/report`, `/artifact` | 현재 화면의 central/inspector 영역, notification, modal, 또는 report 화면에 정보를 표시하고 입력은 비운다. `/project`는 진단만 표시하며 workspace 선택은 `/target <path>` 또는 `/workspace`가 담당한다. |
