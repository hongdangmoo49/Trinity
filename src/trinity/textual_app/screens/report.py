...
REPORT_SECTION_LABELS_KO = {
    ...
    "Central WP Graph": "중앙 작업 패키지 그래프",
    "Local WP Graph": "로컬 작업 패키지 그래프",
...
def _render_overview_meta(meta, *, lang: str = "en") -> str:
    lines = [
        ...
        f"{escape(_status_value(meta.state, lang=lang))}",
...
def _render_overview_snap(snap: WorkflowNexusSnapshot, *, lang: str = "en") -> str:
    lines = [
        ...
        f"[bold]{_field_label('State', lang=lang)}[/bold]: "
        f"{escape(_status_value(snap.state, lang=lang))}",
...
def _package_routing_summary(
    ...
    *,
    ...
    if package.task_kind:
        parts.append(
            f"{_term_label('kind', lang=lang)} "
            f"{escape(display_kind_value(package.task_kind, lang=lang))}"
        )
