...
| `/memory` | `/memory [stats\|compact]` | 로컬 파일 기록 | shared context memory 통계와 bounded projection 압축을 실행한다. |
...
| :--- | :--- | :--- | :--- |
| 로컬/UI 조회 | 없음 | `/help`, `/status`, `/context`, `/model`, `/history`, `/workflow`, `/questions`, `/decisions`, `/packages`, `/subtasks`, `/report`, `/artifact` | 현재 화면의 central/inspector 영역, notification, modal, 또는 report 화면에 정보를 표시하고 입력은 비운다. |
| 로컬 파일 기록 | 없음 | `/save`, `/report save`, `/memory` | 파일 저장 또는 shared projection 압축 후 저장 경로/상태를 notification 또는 central log에 표시한다. |
