...
화면별 modal, central result, picker, empty state 같은 구체적인 UI surface 계약은
[Trinity Slash Command UX Contract](2026-06-07-trinity-slash-command-ux-contract.md)를
기준으로 한다.
...
| `/context` | 없음 | 로컬/UI 조회 | 없음 | 없음 | 현재 active session projection의 goal, synthesis, 질문, decision, work package, execution log 요약을 표시한다. `.trinity/shared.md`나 이전 archive를 직접 읽지 않는다. active session 정보가 없으면 화면별 empty 안내를 표시한다. |
| `/project` | `[workspace]` | 로컬/UI 조회 | 없음 | 없음 | 인자가 없으면 Start/Nexus의 긴 프로젝트 진단을 local command 결과로 표시한다. 하위 명령은 에이전트를 호출하지 않고 workspace picker만 연다. 분석이나 작업 의도는 prompt로 전달해 중앙 에이전트가 판단한다. |
| `/providers` | 없음 | 로컬/UI 조회 | 없음 | 없음 | Textual provider inspector를 연다. provider readiness를 새로 실행하지 않고 현재 snapshot을 표시한다. |
| `/workspace` | 없음 | 로컬/UI 조회 | 없음 | 없음 | target workspace 선택 picker를 연다. 선택된 workspace는 다음 prompt/follow-up 또는 execution target 후보로 사용된다. |
| `/settings` | 없음 | 로컬/UI 조회 | 없음 | 없음 | Textual Workbench 설정 화면으로 전환한다. plain TUI에서는 기본 `trinity` Workbench에서 `/settings` 또는 Ctrl+comma를 사용하라는 안내를 표시한다. |
...
| `/model` | 없음 | 로컬/UI 조회 | 없음 | 현재 Textual selector 모델 override 변경 | Textual Start/Nexus에서 에이전트별 모델 설정 modal을 연다. 선택한 모델은 다음 prompt/follow-up의 model override로만 사용되며, provider를 호출하지 않고 설정 파일에도 저장하지 않는다. |
...
| `/ask` | `<all\|agent[,agent...]> [--model MODEL] <prompt>` | 조건부 workflow 변경 | 조건부 | `.trinity/workflow/session.json`, `events.jsonl` | Start에서는 선택한 agent만 대상으로 workflow를 시작하고, Nexus에서는 선택한 agent만 대상으로 follow-up을 기록한다. `--model`이 있으면 해당 요청의 대상 agent에만 모델 override를 적용한다. |
...
| `/execute-retry` | `[all\|failed\|blocked\|interrupted\|custom\|WP-ID...]` | 명시 실행 | execution | retry package 상태 변경, execution event, package status 변경 | 실패, 차단, 중단된 work package를 대상으로 중앙 retry modal 또는 plain TUI 확인 흐름을 연다. 사용자가 범위와 package를 확정한 뒤 선택된 package만 새 execution worker에 dispatch한다. |
| `/review` | `[wp\|final\|all] [WP-ID...]` | 명시 실행 | execution | review package/result 저장, review event 기록 | 대기 중인 WP 리뷰 또는 최종 프로젝트 리뷰를 실행한다. `wp`는 package별 코드 리뷰, `final`은 프로젝트 전체 리뷰, `all`은 WP 리뷰 후 최종 리뷰를 순서대로 처리한다. |
| `/improve` | `[all\|critical\|high\|AI-ID...\|done\|text]` | 명시 실행 | 조건부 | post-review item/follow-up request 저장, 선택 시 supplemental WP 추가 | final review 이후 `POST_REVIEW_READY` 상태에서만 동작한다. `done`은 workflow를 `DONE`으로 닫고, severity/ID 선택 또는 자유 입력은 action item을 accepted/queued로 바꾼 뒤 `WP-S001` 형식의 supplemental WP를 append한다. |
| `/memory` | `[stats\|compact]` | 로컬 파일 기록 | 없음 | `compact` 시 `.trinity/shared.md` bounded projection 재작성 | 인자가 없거나 `stats`이면 shared context memory 통계와 크기 제한을 표시한다. `compact`는 memory index의 최근 기록과 pinned section으로 `shared.md`를 재생성하고 oversized 원본은 백업한다. |
| `/artifact` | `<memory-id>` | 로컬/UI 조회 | 없음 | 없음 | memory index에 기록된 record id를 받아 kind, agent, work package, artifact path, summary를 표시한다. record가 없으면 에이전트를 호출하지 않고 not found를 표시한다. |
...
| `/improve` | `POST_REVIEW_READY`에서 action item 목록을 보여주거나 선택된 보강 WP를 queue한다. 실행이 필요한 경우에만 execution worker를 시작한다. |
| unknown slash | notification 또는 central log에 오류를 표시하고 composer를 비운다. |

## 공통 registry 설계

현재 구현은 `src/trinity/slash_commands.py`의 공통 registry를 기준으로 한다.
