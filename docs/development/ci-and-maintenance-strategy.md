...
- Package version inspected: `1.0.1088`
- Merged PR range reviewed: #90 through #1249
- Baseline iteration reviewed: #90 through #426
- Maintenance refresh reviewed: #427 through #1249
- Latest refresh reviewed: #1249
...
### #977-#1245: Prompt-Led Workbench Simplification
...
- Reworded Textual new-project brief readiness labels to prompt readiness.
...
- Fixed Start command palette content height and restored the workspace action
  row/button to a Textual-renderable height.
- Kept command palette bottom borders above their composer bottom borders so
  the blue palette frame remains visible.
- Kept the Nexus command palette bottom border above the Nexus composer bottom
  border after keyboard navigation.
- Docked the Nexus command composer while slash commands are open so the
  palette remains inside low terminal heights.
- Compact the Start geometry at low terminal heights so the prompt, workspace
  action, and slash command palette remain visible.
- Extended Start and Nexus compact viewport regression coverage down to
  60x20 terminals.
- Added Start compact geometry resize regression coverage for low-to-standard
  terminal transitions.
- Covered Settings model discovery refreshes after a user-selected discovered
  model so automatic choices preserve the selected model.
- Kept the `/model` settings modal selection visible when live discovery
  refreshes replace the available model list.
- Covered Nexus `/model` settings modal selections through follow-up model
  overrides.
- Covered Settings central-model discovery refreshes so selected central
  models survive automatic choice updates.
- Kept the `/model` settings modal default model at the top when preserving a
  missing selected default choice.
- Kept the `/model` settings modal default model at the top when discovery
...
- Synced Start, Nexus, and open Settings agent controls immediately after
  `/agent` enables or disables a provider.
- Refreshed stale Nexus provider snapshots after `/agent` changes so provider
  panels match the new enabled state even after local slash command output.
- Kept `/agent` enabled/disabled changes session-only when users later press
  Apply in Settings for UI or model defaults.
- Refreshed stale Nexus provider snapshots after Settings model-default Apply
  so provider panels show the newly configured model.
- Added `/settings` as a local Textual slash command so users can open
...
- Added Korean Settings unsaved-status coverage so pending changes keep
...
- Added Settings startup restore coverage for saved agent and central model
  defaults.
- Clarified Settings model-default section titles as saved defaults so they
...
- Preserved pending Settings select values across screen recompose so unsaved
...
- Added README slash command drift coverage so the Korean and English quick
  command tables keep registered Trinity slash commands visible.
...
- Applied saved Settings density and reduced-motion preferences to live Textual
  UI surfaces.
- Applied saved Settings unicode rendering preference to the Start geometry
  glyph mode.
...
- Extended Settings color compatibility styling to Provider Inspector and
  Model Settings modal surfaces.
...
- Removed unused generic Settings motion labels after motion display became
  field-specific.
- Kept Settings UI application under the app-level `Applied` handler instead
  of duplicating theme updates inside the settings screen.
- Localized the Korean Settings applied status so Start/Nexus surface names no
  longer leak into the Korean UI.
- Clarified Korean Settings fallback labels for legacy theme and glyph
  preferences.
- Localized remaining Korean Textual `Nexus` surface labels to `넥서스`.
- Unified the Korean Workspace Picker untracked Git status label with the
  Start/Nexus project context summary wording.
- Localized the Korean execution confirmation work package count summary.
- Aligned the Korean execution confirmation risk label with the existing
  `리스크` terminology used by reports and execution rows.
- Clarified Settings color and logo preference labels so they describe their
  actual terminal color and Start logo scope.
...
- Split Settings preview summaries into UI, central model, and agent model
  sections for better scanability.
- Bounded the Settings preview height and enabled preview scrolling so Apply
...
- Normalized legacy Settings fallback selections in the open Settings screen
  after Apply so controls and preview match the saved current values.
- Renamed the Settings preview heading to current selections so users can tell
...
- Covered Settings model discovery refreshes while the screen already has
  unsaved user selections.
...
- Let Settings preview summaries size to their content instead of fixed rows.
- Let Settings selects and preview use flexible width within the screen.
- Filtered Settings central model choices by the selected central provider.
- Covered Settings central provider reset persistence.
- Marked Settings as unsaved after mounted select changes.
- Preferred the selected central provider when displaying shared central model
  labels in Settings.
...
- Synced the mounted Start geometry immediately after applying Settings logo
  glyph changes.
- Covered Settings visual preference classes across the current Settings
  surface and the Nexus surface after navigation.
- Defaulted new Settings theme preferences to explicit dark mode while keeping
  legacy `system` dark-fallback values readable.
- Defaulted new Settings logo glyph preferences to explicit ASCII mode while
  keeping legacy `auto` ASCII-fallback values readable.
- Defaulted new Settings color profile preferences to explicit default palette
  mode while keeping legacy `auto` default-palette values readable.
- Clarified Settings logo motion labels as animated and reduced motion while
  preserving the stored `normal`/`reduced` values.
...
- Aligned project-intake workspace profile, starter profile, scope candidate,
  validation plan, and read-first checklist notes with the current
  diagnostics/CLI surfaces.
- Aligned startup readiness, target-change refresh, and new-brief prompt notes
  with the current prompt-led Workbench surface.
- Aligned missing-brief, scope-choice, and limited-provider policy notes with
  diagnostics and prompt-led Workbench behavior.
- Aligned provider CLI setup and existing-project diagnostic summary notes with
  `/status` provider notices and `/project` diagnostics.
- Removed the obsolete new-project starter preset button plan; prompt-only
  starter recommendations now cover that onboarding need without adding
  Workbench buttons.
- Aligned the target-name summary note with the current compact `/project`
  diagnostics and CLI status contract instead of persistent Start/Nexus labels.
- Aligned new-project starter and practical brief context notes with prompt-led
  Workbench entry, diagnostics, and provider guidance instead of modal-only
  capture.
...
- Clarified `/project` target guidance so direct `/target <path>` and browsing
  `/workspace` are both visible from diagnostics and invalid argument feedback.
- Clarified slash command summaries so `/workspace` reads as browsing and
...
- Locked Start/Nexus against reintroducing persistent project diagnostic labels;
  `/project` owns readiness, intake, validation, generation, and read-first
  detail.
- Shortened the `/project` diagnostic rollup to `Readiness` / `준비 상태`
  wording.
- Aligned execution confirmation and project-context notes with the current
  `workspace context` contract instead of mode-led wording.
- Neutralized the compact project-intake summary from new/existing wording to
  `recorded` / `기록됨`.
- Reworded execution preflight intake-safety output as project-context status
  with saved-analysis reasons.
- Reworded compact project-intake summaries as project-context summaries.
- Reworded CLI project context path/status messages away from intake wording.
- Reworded CLI project context help and empty-state messages away from intake
  wording.
- Reworded project-context target mismatch summaries away from intake wording.
- Reworded execution confirmation project-context summary items away from
  intake wording.
- Reworded readiness rollup context status away from intake wording.
- Reworded remaining project CLI option help away from intake wording.
...
- Reworded execution preflight and project-context confirmation docs away from
  intake wording.
- Reworded project diagnostics and CLI status drift docs away from intake
  wording.
- Reworded read-first and validation preview docs away from intake wording.
- Reworded new-project preview and brief docs away from intake wording.
- Reworded new-brief and starter-profile docs away from intake wording.
