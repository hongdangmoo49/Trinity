...
1차 구현은 조회/unknown slash가 에이전트 호출 경로로 넘어가는 문제를 막는 데 초점을 둔다.
Textual의 일부 명령은 아직 plain TUI처럼 Rich table/panel을 그대로 렌더링하지 않고
notification, snapshot 갱신, report 화면 전환으로 처리한다.
...
남은 구현 과제는 Textual render target을 plain TUI 수준으로 풍부하게 만드는 것이다.
예를 들어 `/status`는 현재 notification 중심이고, 장기적으로는 central/inspector 영역에
표 형태로 표시하는 것이 좋다.
...
1. Textual 조회 명령의 표현은 아직 notification/snapshot 갱신 중심이며, plain TUI의
   table/panel 수준에는 못 미친다.
