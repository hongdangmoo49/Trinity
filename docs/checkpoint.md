...
- Textual Workbench는 `TextualWorkflowController`를 통해 기존 workflow/orchestrator를
  ...
  화면에 투영한다.
- Trinity 앱 자체 slash command는 공통 registry로 분류된다. 로컬/UI 조회와 세션 설정
  변경 명령은 provider prompt/follow-up으로 전달하지 않고, Textual에서는 중앙
  `Local Command Results`에 결과를 남긴다.
- Textual Start의 `/status`는 workflow를 시작하지 않고 status modal로 표시한다.
  Textual Nexus의 `/status`는 중앙 status 결과를 최신 값으로 교체하며, idle 상태에서
  이전 `shared.md` 합의 결론을 synthesis로 표시하지 않는다.
- `/rounds`, `/agent`, `/caveman`은 현재 프로세스 config만 바꾸며, plain TUI 출력과
  Textual 중앙 로그에 config 파일 미저장 안내를 표시한다.

### 최신 운영 문서

- [v0.12.0 Workflow and Runtime Guide](workflow-v0.10.2-guide.md)
...
- WSL 최신 전체 회귀 기록: `/home/zaemi/.local/bin/uv run pytest -q` -> `1224 passed, 1 warning in 55.31s`
...
| `/context` | 현재 세션의 목표, 합의, 질문, 결정, 작업 패키지 요약 표시 |
