...
- `reviewing`은 review package 또는 pending final review 조건이 있어야 한다.
...
실행 중인 workflow에서 특정 WP가 완료되어도 전체 execution run이 끝나기 전까지
`execution_results`가 session에 fully persisted되지 않을 수 있다. Runtime event는 summary를
남기지만, report가 구조화된 `ExecutionResult`를 기준으로 렌더링한다면 사용자는 "완료 로그는
있는데 보고서가 없다"고 느낄 수 있다.
