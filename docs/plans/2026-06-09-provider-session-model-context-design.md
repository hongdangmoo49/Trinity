...
Codex는 `codex exec resume [SESSION_ID] [PROMPT]`를 지원한다. 단, resume help에는 `--cd`와 `--sandbox` 옵션이 일반 `codex exec`만큼 명확히 노출되지 않는다. 따라서 초기 구현에서는 Codex resume이 cwd/sandbox를 어떻게 상속하는지 contract test를 먼저 작성해야 한다.
...
5. Provider-native continuation은 `workflow_id + agent_name + lane`에 묶는다.
...
- `codex exec resume`이 cwd/sandbox를 어떻게 상속하는지 contract test를 통과하기 전에는 execution lane resume을 보수적으로 비활성화할 수 있다.
...
이번 작업의 목표는 continuity mode를 구현하는 것이다. 다만 Codex execution lane resume은 sandbox/cwd 상속 검증 전까지 read-only deliberation/review부터 적용한다.
...
- Provider session id는 `workflow_id + agent + lane + access + cwd_hash`에 매핑한다.
...
- Model display는 `configured_model`과 `actual_model`을 분리한다.
